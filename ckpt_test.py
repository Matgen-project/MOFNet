...
def test(model, data_loader, mean, std):
    ...
    while data is not None:
        ...
        output = model(node_features, batch_mask, adjacency_matrix, distance_matrix, None, global_features, 1)
    ...
    return {'MAE':mae, 'RMSE':rmse, 'PCC':pcc, 'sMAPE':smape}, futures, ys
...
if __name__ == '__main__':
    ...
    f = f[idx_list]
    y = y[idx_list]
    futures_all = {pres:[] for pres in p}
    ys_all = {pres:[] for pres in p}
    ...
    for fold_idx in range(1,fold_num + 1):
        ...
        for pres in p:
            ...
            test_errors[pres], futures_tmp, ys_tmp = test(model, test_loader, mean, std)
            futures_all[pres].append(futures_tmp)
            ys_all[pres].append(ys_tmp)
    ...
    for pres in p:
        futures_all[pres] = np.concatenate(futures_all[pres])
        ys_all[pres] = np.concatenate(ys_all[pres])
        for _ in test_errors_all[0][pres].keys():
            err_mean = np.mean([__[pres][_] for __ in test_errors_all])
            err_std  = np.std([__[pres][_] for __ in test_errors_all])
            print('Pressure {}, Test {} of {:02d}-Folds: {:.7f}({:.7f})'.format(pres, _,fold_num,err_mean,err_std))
            logger.info('Pressure {}, Test {} of {:02d}-Folds: {:.7f}({:.7f})'.format(pres, _,fold_num,err_mean,err_std))

    with open(model_params['save_dir'] + f"/{model_params['gas_type']}_{model_params['pressure']}/test_result.p",'wb') as f:
        pickle.dump((futures_all, ys_all), f)
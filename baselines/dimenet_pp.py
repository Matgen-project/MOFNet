...
class EmbeddingBlock(torch.nn.Module):
    def __init__(self, num_input, num_radial, hidden_channels, act=swish):
        super().__init__()
        self.act = act

        self.emb = nn.Linear(num_input, hidden_channels)
...
class DimeNetPlusPlus(torch.nn.Module):
    ...
    def __init__(
        ...
        self.emb = EmbeddingBlock(num_input, num_radial, hidden_channels, act)
    ...
    def forward(self, node_features, batch_mask, pos, adj):
        """"""
        # edge_index = radius_graph(pos, r=self.cutoff, batch=batch)
        batch_size, n_nodes, in_node_nf = node_features.shape
        edge_index, _ = dense_to_sparse(adj)
        
        node_features = node_features.reshape(-1, in_node_nf)
        pos = pos.reshape(-1, 3)
        j, i = edge_index
        dist = (pos[i] - pos[j]).pow(2).sum(dim=-1).sqrt()

        _, _, idx_i, idx_j, idx_k, idx_kj, idx_ji = self.triplets(
            edge_index, num_nodes=node_features.size(0)
        )
        ...
        # Embedding block.
        x = self.emb(node_features, rbf, i, j)
        ...
        #     return target, out
        return P.view(-1, n_nodes, self.hidden_channels)

...
def test(model,data_loader,mean,std, img_dir,names, p_ori):
    ...
    for data in tqdm(data_loader):
        ...
        pres_itp = pressure_itp.cpu().detach().numpy().reshape(-1) + p_ori
        mae = np.mean(np.abs(y_tmp - futures_tmp))
        if mae > 0.5:
            continue
        ...
        plt.legend(handles=[l1, l2, l3],labels=['label','prediction','interpolation'],loc='best')
        plt.savefig(f'{img_dir}/{names[batch_idx]}_mae={mae:.4f}.png')
        plt.cla()
        ans[names[batch_idx]] = {
            'log_pressure':pres,
            'label':y_tmp,
            'pred':futures_tmp,
            'log_pressure_interpolated':pres_itp,
            'pred_interpolated':futures_tmp_itp
        }
#        ys += list(y_tmp)
...
def ensemble_test(models,data_loader, mean, std, img_dir, names):
    os.makedirs(img_dir,exist_ok=True)
    for model in models:
        model.eval()
    error = 0
...
#    f = np.array(f)
    ...
    img_dir = f"images/csd/{model_params['gas_type']}"
    predict_res = []
    ...
    f = f[idx_list]
    y = y[idx_list]
    names = applyIndexOnList(names, idx_list)
    ...
    _, _, test_idx = splitdata(len(X),10,1)
    ...
    test_res = test(models[0],test_loader,mean,std, img_dir, applyIndexOnList(names,test_idx), model_params['pressure'])
    with open(os.path.join(img_dir,'res.p'),'wb') as f:
